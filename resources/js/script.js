...
// Function declaration for clamping windoww sizes

const clamp = (num, min, max) => {
  return Math.min(Math.max(num, min), max);
};

// Function declaration for turning dvw values to rem values.

const dvwToRem = (num) => {
  windowWidth = window.innerWidth || document.documentElement.clientWidth;
  num = num * (windowWidth / 100); //Converts dvw to px.
  return num / 16; //Converts px to rem.
};

// Function declaration for turning rem values to dvw values.

const remToDvw = (num) => {
  num = num * 16; //Converts rem to px.
  return (num / windowWidth) * 100; //Converts px to dvw.
};
...
const portImgHover = (event) => {
  ...
  event.currentTarget.style.width = "clamp(6.4rem, 11dvw, 8rem)";
};
...
const portImgOut = (event) => {
  ...
  event.currentTarget.style.width = "clamp(5rem, 10dvw, 6rem)";
};

//Portfolio image clicked
const portImgClick = (event) => {
  if (!portWindowOpen) {
    //First check to se if a portfolio window is already open
    ...
    img.style.filter = "grayscale(0%)";
    portItem.style.width = "clamp(7rem, 12dvw, 8rem)";
    ...
    else pHeight = pHeight / 16 + 6; //Check and set min-height

    // Start showing the portfolio background and set the hight to earlier
    // calculations

    const portWindowWidth = clamp(dvwToRem(40), 24, 37);
    applyStyles(portWindow, {
      transitionTimingFunction: "ease-in-out",
      transition: "height 0.3s, opacity 1.5s",
      opacity: "1",
      height: pHeight + "rem",
      width: remToDvw(portWindowWidth * 1.4) + "dvw",
      transform: "translateX(" + remToDvw(portWindowWidth * -0.2) + "dvw)",
    });
    ...
    //If portfolio window is open and the clicked portfolio is clicked again.
...
const abortPortfolio = () => {
  //Close portfolio. Can be called by exitbox click and click on active portfolio image
  ...
  portItem.style.width = "clamp(5rem, 10dvw, 6rem)";

  setTimeout(() => {
    //Giv the transitions time before turning of displays
    applyStyles(portfolioBody, {
      marginTop: "-10rem",
      display: "none",
    });
...
const pressButton = (event) => {
  ...
  if (element.tagName !== "LI") {
    //Checkes to se if the event target is an LI of our menu UL
    return;
  }

  switch (
    element.id //Match event target with right button and then send right id to the scroll to function
  ) {
    case "portfolioBtn":
